...
RSpec.describe ValidateLoanVersionInteractor do
  ...
  it 'validates loan version' do
    context = ValidateLoanVersionInteractor.call(loan_version: loan_version_ratified, user: administrator,
                                                 validation_object: 'version')
    expect(context).to be_success

    new_loan = loan.reload
    loan_version = new_loan.active_loan_version
    expect(loan_version.status).to eq(LoanVersion::STATUS_RATIFIED)
  ...
  it 'sends validation email' do
    ...
    expect(email.subject).to eq(I18n.t('mailers.loan_version_validated.subject', meta: '[test]'))
    expect(email.to).to eq([investment_manager.email])
  end

  it 'validates the repayment calendar if calendar exists' do
    context = ValidateLoanVersionInteractor.call(loan_version: invested_loan_version_invested, user: administrator,
                                                 validation_object: 'version')
    expect(context).to be_success

    new_loan = invested_loan.reload

    repayment_calendar = new_loan.active_repayment_calendar
    expect(repayment_calendar.version_status).to eq(LoanVersion::VERSION_STATUS_VALIDATED)
    expect(repayment_calendar.validation_user).to eq(administrator)
  end

  it 'creates a log line for calendar' do
    context = ValidateLoanVersionInteractor.call(loan_version: invested_loan_version_invested, user: administrator,
                                                 validation_object: 'version')
    expect(context).to be_success
